...
exports.getSpecificCompany = (APP, req, callback) => {
    ...
    async.waterfall(
        [
            ...
            function getInfoLogsUser(data, callback) {
                let query;
                Promise.all(
                    data.logs.map(x => {
                        if (x.level == 3) {
                            let { employee } = APP.models.company[`${process.env.MYSQL_NAME}_${x.company}`].mysql;
                            query = employee;
                        }
                        ...
                        return query
                            ...
                            .then(res => {
                                obj.level = x.level == 2 ? 'admin company' : x.level == 3 ? 'employee' : 'admin ceklok';
                                obj.user_id = res.name;
        ...
        [
            ...
            function getEmployeeInfo(data, callback) {
                ...
                employee
                    ...
                    .then(res => {
                        if (res.length == 0)
                            return callback({ code: 'NOT_FOUND', message: 'Employee tidak ditemukan!' });
