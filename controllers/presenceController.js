...
exports.generateDailyPresence = (APP, req, callback) => {
    let {
        ...
        schedule,
        schedule_switch
    } = APP.models.company[`${process.env.MYSQL_NAME}_${req.body.company}`].mysql;
    async.waterfall(
        [
                                ...
                                res.map((data, index) => {
                                    ...
                                    if (
                                        ...
                                        (data.presence_setting_id == 1 ||
                                            (data.check_in == '00:00:00' && data.check_out == '00:00:00'))
                                    ) {
                                                        ...
                                                        schedule_switch
                                                            ...
                                                            .then(resSwitch => {
                                                                if (resSwitch.length == 0) {
                                                                    // Data Absent Cuti Switch Not Found == Absent
                                                                    presence
                                                                        .update(
                                                                            {
                                                                                presence_setting_id:
                                                                                    result.status[4].id, // 'A'
                                                                                total_minus: data.schedule.work_time
                                                                            },
                                                                            {
                                                                                where: {
                                                        ...
                                                        if (x.type == 0) {
                                                            ...
                                                            if (x.time_total > data.total_over) {
                                                                params = {
                                                                    ...
                                                                    total_minus: APP.time.timeDuration([
                                                                        x.time_total,
                                                                        data.total_minus
                                                                    ]),
                                                                    total_over: '00:00:00'
                                                                ...
                                                                params = {
                                                                    ...
                                                                    total_over: APP.time.timeDuration([
                                                                        x.time_total,
                                                                        data.total_over
                                                                    ])
                                                                };
                                        ...
                                        .then(found => {
                                            if (found.length == 0) {
                                                ...
                                                return found.map((x, index2) => {
                                                    ...
                                                    let workTotal = moment.duration(
                                                        APP.time.timeDuration(totalTime[data.id])
                                                    );
                                                    let workMinus = moment.duration(
                                                        APP.time.timeDuration(totalMinus[data.id])
                                                    );
                                                    let workOver = moment.duration(
                                                        APP.time.timeDuration(totalOver[data.id])
                                                    );
...
exports.checkInOutProcess = (APP, req, callback) => {
    ...
    async.waterfall(
        [
            ...
            function checkScheduleSwitch(result, callback) {
          ...
          SELECT 
              ...
              schreq.id AS 'req_schedule_id',
              schreq.name AS 'req_schedule_name',
              schreq.work_day AS 'req_schedule_work_day',
              schreq.work_time AS 'req_schedule_work_time',
              ...
              schtarget.name AS 'target_schedule_name',
              schtarget.work_day AS 'target_schedule_work_day',
            ...
            INNER JOIN
              ${req.user.db}.employee req
            ON
              switch.user_id = req.id
            INNER JOIN
              ${req.user.db}.employee target
            ON
              switch.target_user_id = target.id
            INNER JOIN
              ${req.user.db}.schedule schreq
            ON
              req.schedule_id = schreq.id
            INNER JOIN
              ${req.user.db}.schedule schtarget
            ON
              target.schedule_id = schtarget.id
            WHERE
              (switch.user_id = ${req.user.id} OR switch.target_user_id = ${req.user.id})
            AND
              (switch.date = '${today}' OR switch.target_user_date = '${today}')
            ORDER BY 
              id DESC 
            LIMIT 1
            `
                    ...
                    .then(res => {
                        if (res[0].length == 0) {
                            ...
                            if (res[0][0].date == today) {
                                obj.id = res[0][0].req_schedule_id;
                                obj.work_day = res[0][0].req_schedule_work_day.split(',');
                                obj.name = res[0][0].req_schedule_name;
                                obj.work_time = res[0][0].req_schedule_work_time;
                                obj.check_in_start = res[0][0].req_schedule_check_in_start;
                                obj.check_in_end = res[0][0].req_schedule_check_in_end;
                                obj.check_out_start = res[0][0].req_schedule_check_out_start;
                                obj.check_out_end = res[0][0].req_schedule_check_out_end;
                            }

                            // sebagai yang dituju
                            if (res[0][0].target_user_date == today) {
                                obj.id = res[0][0].target_schedule_id;
                                obj.work_day = res[0][0].target_schedule_work_day.split(',');
                                obj.name = res[0][0].target_schedule_name;
                                obj.work_time = res[0][0].target_schedule_work_time;
                                obj.check_in_start = res[0][0].target_schedule_check_in_start;
                                obj.check_in_end = res[0][0].target_schedule_check_in_end;
                                obj.check_out_start = res[0][0].target_schedule_check_out_start;
                                obj.check_out_end = res[0][0].target_schedule_check_out_end;
                            }
                    ...
                    .catch(err => {
                        console.log(err, 'Error checkScheduleSwitch');
                        callback({
                            code: 'ERR_DATABASE',
                            message: 'Error checkScheduleSwitch',
                            data: err
                        });
                    });
            },
            ...
            function checkSchedule(result, callback) {
                if (result.schedule) return callback(null, result);
                ...
                employee
                    .findOne({
                        attributes: ['id', 'schedule_id', 'nik', 'name'],
                        include: [
                            {
                                model: schedule,
                                required: true
                    ...
                    .then(res => {
                        if (res === null) {
                            ...
                            let workDay = res.schedule.work_day;
                    ...
                    .catch(err => {
                        console.log(err, 'Error checkSchedule');
                        callback({
                            code: 'ERR_DATABASE',
                ...
                if (time.isBetween(checkInStart, checkInEnd)) {
                    presence
                        ...
                        .then(res => {
                            ...
                            res.update({
                                check_in_device_id: req.useragent.isMobile ? result.device.id : null,
                ...
                //check out
                    ...
                    presence
                        ...
                        .then(res => {
                            ...
                            let params = {
                                check_out_device_id: req.useragent.isMobile ? result.device.id : null,
                                ...
                                total_minus:
                                    totalTime < result.schedule.work_time
                                        ? moment
                                              .utc(
                                                  moment(result.schedule.work_time, 'HH:mm:ss').diff(
                                                      moment(totalTime, 'HH:mm:ss')
                                                  )
                                              )
                                              .format('HH:mm:ss')
                                        : '00:00:00',
                                        ...
                                        ? moment
                                              .utc(
                                                  moment(totalTime, 'HH:mm:ss').diff(
                                                      moment(result.schedule.work_time, 'HH:mm:ss')
                                                  )
                                              )
                                              .format('HH:mm:ss')
                                        : '00:00:00',
...
exports.getDetailCheckInOut = (APP, req, callback) => {
    if (!req.body.id) {
        return callback({
            code: 'INVALID_REQUEST',
    ...
    let params = ` WHERE presence.id = ${req.body.id}`;

    if (req.user.level === 3) {
        params = ` WHERE presence.user_id = ${req.user.id} AND presence.id = ${req.body.id}`;
    }
