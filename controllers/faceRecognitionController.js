        ...
        [
            function uploadNewImage(callback) {
                try {
                    ...
                    Promise.all(
                        arr.map((x, i) => {
                            return APP.cdn.writeFileCDN(x, dir + `${i + 1}.jpg`).then(res => {
                                if (res.error == true) {
                                    throw new Error();
                                } else {
            ...
            function isMobileAndroid(data, callback) {
                if (req.headers['platform'] !== 'Mobile') return callback(null, data);
                if (!req.body.training)
                    return callback({ code: 'INVALID_REQUEST', message: 'Kesalahan pada parameter training!' });
                ...
                APP.cdn.writeFileCDN(req.body.training.toString(), `${dir}/android_training.txt`, 'utf8').then(res => {
                    if (res.error == true) {
                        throw new Error();
                    } else {
                        console.log('Mobile Device');
                        data.android_dir = `${dir}/android_training.txt`;
                        callback(null, data);
                    }
                });
            },
                ...
                Promise.all(
                    labels.map(async label => {
                        ...
                        for (let i = 1; i <= 1; i++) {
                            ...
                            let img = await canvas.loadImage(`${process.env.CDN_HOST}${data.dir.slice(8)}${i}.jpg`);
                            console.log(img);
                    ...
                    .then(arr => {
                        ...
                        Promise.all(
                            arr.map(x => {
                                ...
                                return APP.cdn.writeFileCDN(json, `${dir}${x.label}.json`, 'utf8').then(res => {
                                    if (res.error == true) {
                                        throw new Error();
                                    } else {
