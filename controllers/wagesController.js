...
exports.addWages = (APP, req, callback) => {
    ...
    async.waterfall(
        [
            function validationRequest(callback) {
                if (!grade_id || !job_title_id || !branch_id || !minimum_nominal || !maximum_nominal)
                    return callback({
                ...
                wages
                    ...
                    .catch(err => {
                        callback({
                            code: 'ERR_DATABASE',
                            message:
                                'Database bermasalah, mohon coba kembali atau hubungi tim operasional kami ( insert )',
                            data: err
                        });
                    });
            }
                ...
                wages
                    ...
                    .catch(err => {
                        callback({
                            code: 'ERR_DATABASE',
                            message:
                                'Database bermasalah, mohon coba kembali atau hubungi tim operasional kami ( update )',
                            data: err
                        });
                    });
            }
...
exports.calulatorWages = (APP, req, callback) => {
    ...
    async.waterfall(
        [
            function validationRequest(callback) {
                if (!grade_id || !job_title_id || !branch_id)
                    return callback({
                        code: 'INVALID_REQUEST',
                        message: 'Kesalahan parameter ( All )'
                    });
                ...
                if (!deduction || !Array.isArray(deduction) || deduction.length == 0)
                    return callback({
                        code: 'INVALID_REQUEST',
                ...
                Promise.all(
                    income.map(x => {
                        return income_deduction
                            ...
                            .then(res => {
                                ...
                                if (res.length > 0) {
                                    data.income +=
                                        res[0].denomination == 1
                                            ? parseInt(res[0].nominal)
                                            : (res[0].nominal * data.middle_value) / 100;
                    ...
                    .catch(err => {
                        callback({
                            code: 'ERR_DATABASE',
                            message:
                                'Database bermasalah, mohon coba kembali atau hubungi tim operasional kami ( promise income )',
                            data: err
                        });
                    });
            },
                ...
                Promise.all(
                    ...
                    .catch(err => {
                        ...
                        callback({
                            code: 'ERR_DATABASE',
                            message:
                                'Database bermasalah, mohon coba kembali atau hubungi tim operasional kami ( promise deduction )',
                            data: err
                        });
                    });
            }
