                    ...
                    Promise.all(
                        files.map(file => {
                            ...
                            return APP.db.sequelize
                                .query(
                                    `CREATE TABLE ${data.dbName}.${tableName} LIKE ${process.env.MYSQL_NAME}.${tableName}`
                                )
                                .then(() => {
                                    x.map(model => {
                                        let Model = APP.db
                    ...
                    try {
                        if (!req.files || Object.keys(req.files).length === 0) {
                            return callback({
                                ...
                                id: 'PVS01',
                                message:
                                    'Mohon maaf terjadi kesalahan, tidak ada gambar dipilih atau pilih gambar sekali lagi'
                            });
                        }
                                    ...
                                    Promise.all(
                                        ...
                                        .then(arr => {
                                            user_type_feature
                                                ...
                                                .catch(err => {
                                                    console.log(
                                                        'Error user type features setDefaultValueUserType',
                                                        err
                                                    );
                                                    callback({
                                                        code: 'ERR_DATABASE',
